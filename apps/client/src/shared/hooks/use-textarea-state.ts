 ...
 *
 * @returns {[string, ChangeEventHandler<HTMLTextAreaElement>]}
 * - 현재 입력값 상태 (string)
 * - 입력값 변경 핸들러 함수 (textarea 요소의 onChange에 직접 연결 가능)
 *
 * @example
 * function Example() {
 *   const [value, onChange] = usetextAreaState('');
 *   return <textarea type="text" value={value} onChange={onChange} />;
 * }
 */
